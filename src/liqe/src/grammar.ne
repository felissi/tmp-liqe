...
expression -> unquoted_value {% (data, start, reject) => {
...
range ->
    range_open (decimal | unquoted_value) " TO " (decimal | unquoted_value) range_close {% (data, start) => {
    return {
      location: {
        start,
      },
...
unquoted_value ->
  [a-zA-Z\u4E00-\u9FFF\d_*?@#$] [a-zA-Z\.\-\u4E00-\u9FFF\d_*?@#$]:* {% d => d[0] + d[1].join('') %}
